...
class ListingsController extends Controller
{
    ...
    public function importListing() {
        ...
        foreach ($all_mls as $curr_mls_id => $curr_mls){
            Log::info("Listings Cron Now Started = ".date("Y-m-d"));
            ...
            foreach ($property_class as $className => $classconfig) {
                ...
                $rets_query = $date_query.",(MLSTATUS=A,CS,UC,US)";
                $rets_query = "(MLSTATUS=A,CS,UC,US)";
                ...
                //dd($property_resource,$className);
                //$rets_metadata      = $rets->GetTableMetadata($property_resource, $className);
                //$mappings = $this->createMapping($rets_metadata);
                //dd($mappings,$className);
                //echo "<br>".$sql = $this->create_table_sql_from_metadata($property_table_name, $rets_metadata, 'Matrix_Unique_ID');
                //dd(1);
                ...
                if ($total_property_count > 0) {
                    ...
                    for ($lcp = 0; $lcp <= $lc; $lcp++) {
                        ...
                        foreach ($search as $record) {
                            ...
                            $result = RetsPropertyData::updateOrCreate([$key_field => $record[$key_field]], $property_data);
