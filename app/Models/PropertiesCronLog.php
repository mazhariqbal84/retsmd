...
{
    use HasFactory;
    protected $table = "properties_cron_log";
    protected $fillable = [
        "cron_file_name",
        "property_class",
        "rets_query",
    ...
    public function get_properties_end_time_for_last_sucess_cron($cron_file_name,$get_time=true,$table_name="properties_cron_log",$className=1,$mls_no="") {
        ...
        DB::enableQueryLog();
        //dd($cron_file_name,$mls_no,$className);
        $query =  \App\Models\PropertiesCronLog::where("success",1)
    ...
    public function get_properties_end_time_for_last_sucess_cron_for_mls_ids($file_name,$className,$curr_mls_id) {
        ...
        DB::statement("SET SQL_MODE=''");
        // DB::enableQueryLog();
        $query =  DB::table('properties_cron_log')->where("success",1)
